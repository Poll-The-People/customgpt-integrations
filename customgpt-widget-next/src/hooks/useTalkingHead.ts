 ...
 */
function loadScriptFromCDN(url: string): Promise<void> {
...
export const useTalkingHead = (
  ...
  const loadTalkingHeadLibrary = useCallback(async (): Promise<boolean> => {
    ...
    try {
      // Load from CDN via script tag (Next.js compatible)
      console.log('[Avatar] 📦 Loading from CDN via script tag...');
      await loadScriptFromCDN(TALKINGHEAD_CDN_URL);

      // Access global TalkingHead class
      const module = (window as any).TalkingHead ? { default: (window as any).TalkingHead } : null;
      if (!module) {
        throw new Error('TalkingHead not found on window object');
      }
      ...
      const TalkingHeadClass = module.default || (module as any).TalkingHead;
